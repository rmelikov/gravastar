...
* **repair vector** — direction/magnitude derived from $y=\operatorname{arctanh}(x)$ to move a failing proposal toward $\Phi_{\min}$ without violating floors.
...
* **Observer-equivalence group $\mathcal{G}$.**
...
* **G-floor.** Re-descriptions $g\in\mathcal G$ must *not* flip the WITNESS verdict; any compliant $g$ that does so defeats the claim (supersession).
...
* **G-invariance.** Re-descriptions in $\mathcal G$ must not change the pass/fail outcome after rounding.
...
5. The artifact is **G-invariant**: any admissible $g\in\mathcal G$ re-description yields the **same $\delta$** and a content-equivalent artifact under the pinned canonicalizer.
...
* **G-floor (§5.1):** All $g\in\mathcal G$ tests reuse the **same seed vector**; seed changes across $g$ are non-compliant.
...
* **Decision outputs**
  ...
  * `g_coverage`: summary of $\mathcal{G}$-tests with REPLAY-RFD IDs (§5.1)
...
7. **G-invariance**: admissible re-descriptions in $\mathcal{G}$ (units/locale/paraphrase) leave the artifact’s **content** invariant after canonicalization.
...
* **Admissible observers.** The universe $\mathcal O_{\text{adm}}$ is defined by the declared observer-equivalence group $\mathcal G$ (units, encodings, tokenization, locale, paraphrase, ordering) and the receipt-true method (§5.1, §7).
...
3. **Gauge risk.** Coverage measured in one encoding/locale can mislead if $\mathcal G$-canonicalization is weak; floors—not panel weights—secure invariance.
...
**Disambiguation.** REPLAY-RFD is an **audit/repro** procedure, **not** the Cook–Mertz *Tree Evaluation* problem (§7.3).
...
**Who may join.**
Anyone who accepts the declared observer-equivalence group $\mathcal G$ and uses the pinned method (**III.json**, **Receipt v2**, **ScanProtocol v1**, **REPLAY-RFD**). No identity, affiliation, or locale filtering; $\mathcal G$ canonicalizes encodings, units, tokenization, and ordering (§5.1).
...
**Step 1 – Canonicalize.** Apply $\mathcal G$ canonicalizers (units/locale/tokenization/ordering) to inputs prior to feature extraction; verify that re-descriptions leave rounded diagnostics unchanged.
...
* **Unit/scale poisoning.** Report in psi, compute in Pa; mix °C/°K; embed implicit factors (e.g., “%” vs “fraction”).
  **Detector.** Canonicalizer $\mathcal{C}_{\mathcal{G}}$ maps all inputs to a pinned canonical form; compute

  $$
  \Delta_g=\left|J(\mathcal{C}_{\mathcal{G}}(g\!\cdot\!s),\mathcal{C}_{\mathcal{G}}(g\!\cdot\!a)) - J(\mathcal{C}_{\mathcal{G}}(s),\mathcal{C}_{\mathcal{G}}(a))\right|
  $$

  over a representative $g\in\mathcal{G}_{\text{test}}$; require $\Delta_g<\eta$ at III.json precision.
...
* **Paraphrase/tokenization attacks.** Synonyms, punctuation, or zero-width joiners that alter token boundaries in learned models.
  **Detector.** Tokenization canonicalizer (version-pinned) + paraphrase equivalence harness; require identical rounded diagnostics pre/post $g\in\mathcal{G}_{\text{ling}}$.
...
**Admissible repair.** Publish the canonicalization spec & tests in Receipt v2; re-emit artifacts with canonical bytes; extend $\mathcal{G}_{\text{test}}$ set.
...
Gravastar’s audit primitive **REPLAY-RFD** (Replay-to-First-Divergence) is a *procedural* guarantee: any admissible observer can regenerate our bytes and either corroborate or defeat a claim. It is **not** a new complexity result. Still, it has a natural **lineage** in the theory of evaluating structured computations, especially the **Tree Evaluation** paradigm and two modern advances: **Cook–Mertz** (near-logspace evaluation) and **Williams** (time simulation via sequences of Tree Evaluation instances). This section explains the relationship so readers see what we **borrow** (design ideas) and what we explicitly **do not claim** (asymptotic bounds).

---

### What Tree Evaluation gives us conceptually

* **Model.** In Tree Evaluation (TE), a rooted tree’s leaves hold inputs; each internal node holds a small function; the root’s value is determined by composing children up to the root.
...
* **Advance.** Cook–Mertz showed that certain Tree Evaluation families are solvable in **space $O(\log n\cdot\log\log n)$** (near-logspace) while maintaining correct evaluation order.
...
* **Advance.** Williams shows that simulating time-bounded computations can be reduced to a sequence of **Tree Evaluation** instances, leveraging Cook–Mertz as a subroutine, with nontrivial **time/space trade-offs**.
...
* **Equivalence under $\mathcal{G}$.** All nodes must accept **canonicalized** inputs produced by the $\mathcal{G}$ loaders (units/locale/tokenization/ordering) so audit remains gauge-invariant.

---

### Disambiguation with receipts and floors

* **Different objects.** “Tree Evaluation” (problem) ≠ “REPLAY-RFD” (procedure). We cite TE, Cook–Mertz, and Williams for **intellectual lineage** only.
...
* **Counterexample welcome.** If an admissible observer constructs a **compliant counterexample** (e.g., a $\mathcal{G}$ transform that flips a rounded gate value, or an RFD divergence at a claimed content-bound node), the claim is **defeated** (§4.3).
...
* **Gauge invariance.** Decisions are **invariant under admissible re-descriptions** declared in $\mathcal G$ (units/encodings/ordering/tokenization/locale), verified by the **G-floor**.
...
* **No complexity bounds.** **REPLAY-RFD** is an **audit procedure**, not a claim about Tree Evaluation, Cook–Mertz, or Williams-style asymptotics; we make **no** space/time optimality guarantees.
...
* **Control plane.** Registry of **III.json** versions, **Receipt v2** objects, PRL (Public Revocation List), supersession DAG, rounding/tie policies, and $\mathcal G$ canonicalizer specs.
...
* **Fuzz re-descriptions.** In CI, run a $\mathcal G$-fuzzer (synonyms, decimal conventions, Unicode confusables, column permutations) and require unchanged rounded diagnostics.
...
* **Enterprise.** Multi-site ingestion; centralized control plane; RFD workers autoscaled; deterministic GPU allowed for non-gate steps; strong CI and $\mathcal G$-fuzzing.
...
* **Locale breaks invariance.** Add $\mathcal G$ canonicalizer tests; re-emit artifacts.
...
* Loss of invariance under admissible $\mathcal G$ transforms.
...
* **Auditor MUST:** reproduce cards via REPLAY-RFD; re-fit $\hat c$; re-phase grids; attempt $\mathcal G$ re-descriptions; file counterexample if any criterion fails.
...
* **Canonicalize prompts/contexts** via the $\mathcal{G}$-loaders (units, locale, tokenization, ordering).
...
function TVLM_ANSWER(prompt, ctx, III, tools):
    P = CANONICALIZE_PROMPT(prompt, 𝔾); C = CANONICALIZE_CTX(ctx, 𝔾)
...
* **III.json (pinned manifest).** Budgets, thresholds, precisions, tie policy, $\Phi_{\min}$, $\Phi_{\text{neutral}}$, $\Phi_{\max}$, decode spec, block/ladder sizes, rounding table, and $\mathcal G$ definitions.
...
* **Test vectors.** $\mathcal G$ canonicalization fuzz set (units, locale, tokenization, ordering, Unicode confusables) and timing phasing files for the **three-grid alias refuter**.
...
\[1] Clauser, J. F.; Horne, M. A.; Shimony, A.; Holt, R. A. “Proposed Experiment to Test Local Hidden-Variable Theories.” *Physical Review Letters* **23**, 880–884 (1969). doi:10.1103/PhysRevLett.23.880. ([Physical Review][1])

\[2] Cook, S.; McKenzie, P.; Wehr, D.; Braverman, M.; Santhanam, R. “Pebbles and Branching Programs for Tree Evaluation.” arXiv:1005.2642 (2010). ([arXiv][2])

\[3] Cook, J.; Mertz, I. “Tree Evaluation Is in Space $O(\log n \cdot \log\log n)$.” In *Proceedings of STOC 2024*. ACM (2024). doi:10.1145/3618260.3649664. ([ACM Digital Library][3])

\[4] Williams, R. R. “Simulating Time With Square-Root Space.” arXiv:2502.17779 (2025). ([arXiv][4])

\[5] Su, W. P.; Schrieffer, J. R.; Heeger, A. J. “Solitons in Polyacetylene.” *Physical Review Letters* **42**, 1698–1701 (1979). doi:10.1103/PhysRevLett.42.1698. ([Physical Review][5])

\[6] Su, W. P.; Schrieffer, J. R.; Heeger, A. J. “Soliton excitations in polyacetylene.” *Physical Review B* **22**, 2099–2111 (1980). doi:10.1103/PhysRevB.22.2099. ([Physical Review][6])

\[7] Efroni, Y.; Ilani, S.; Meir, Y. “Topological transitions and fractional charges induced by strain and a magnetic field in carbon nanotubes.” arXiv:1608.05976 (2016; v3 2017). ([arXiv][7])

\[8] Allan, D. W. “Statistics of Atomic Frequency Standards.” *Proceedings of the IEEE* **54**, 221–230 (1966). doi:10.1109/PROC.1966.4634. ([ADS][8])
...
\[12] **Brain-CA patent (teleomorphic/CA framing).** *Artificial intelligence based on cellular automata.* US Patent **11,847,386 B1** (Dec 19, 2023) (and divisional **12,050,846 B1**, 2024). Assignee: Brain-CA Technologies, Inc. ([Google Patents][9])

\[13] **Brain-CA whitepaper.** “The BRAIN-CA™ Estimator and The Cincinnati Algorithm.” Technical whitepaper (June 2024). Brain-CA Technologies. ([Brain-CA Technologies][10])

\[14] (Context note) Brain-CA press announcement summarizing patent family and “teleomorphic” paradigm (Aug 2024). ([Brain-CA Technologies][11])

**Disambiguation.** *REPLAY-RFD (Replay-to-First-Divergence) in this paper is an audit/repro procedure; it is distinct from the Tree Evaluation problem referenced in \[2–4].*

[1]: https://link.aps.org/doi/10.1103/PhysRevLett.23.880?utm_source=chatgpt.com "Proposed Experiment to Test Local Hidden-Variable Theories"
[2]: https://arxiv.org/abs/1005.2642?utm_source=chatgpt.com "Pebbles and Branching Programs for Tree Evaluation"
[3]: https://dl.acm.org/doi/10.1145/3618260.3649664?utm_source=chatgpt.com "Tree Evaluation Is in Space 𝑂 (log 𝑛 · log log 𝑛)"
[4]: https://arxiv.org/abs/2502.17779?utm_source=chatgpt.com "Simulating Time With Square-Root Space"
[5]: https://link.aps.org/doi/10.1103/PhysRevLett.42.1698?utm_source=chatgpt.com "Solitons in Polyacetylene | Phys. Rev. Lett."
[6]: https://link.aps.org/doi/10.1103/PhysRevB.22.2099?utm_source=chatgpt.com "Soliton excitations in polyacetylene | Phys. Rev. B"
[7]: https://arxiv.org/abs/1608.05976?utm_source=chatgpt.com "Topological transitions and fractional charges induced by ..."
[8]: https://ui.adsabs.harvard.edu/abs/1966IEEEP..54..221A/abstract?utm_source=chatgpt.com "Statistics of atomic frequency standards - ADS"
[9]: https://patents.google.com/patent/US11847386B1/en?utm_source=chatgpt.com "Artificial intelligence based on cellular automata"
[10]: https://brain-ca.com/wp-content/uploads/2024/06/The-BRAIN-CA%E2%84%A2-Estimator-and-The-Cincinnati-Algorithm.pdf?utm_source=chatgpt.com "The BRAIN-CA™ Estimator and The Cincinnati Algorithm"
[11]: https://brain-ca.com/brain-ca-technologies-secures-patents-for-innovative-ai-based-on-cellular-automata/?utm_source=chatgpt.com "Brain-CA Secures Patents | Cellular Automata AI Innovation"
...
**Standing context.**
Fix a pinned manifest **III.json** containing: rounding precisions, tie policy, $\Phi_{\min}$, $\Phi_{\text{neutral}}$, $\Phi_{\max}$, decode spec, time budgets $\Pi$, RG-grid parameters, and the observer-equivalence group $\mathcal G$. A **Receipt v2** bundle $B$ contains immutable, content-addressed inputs (raw bytes), SBOMs, canonicalizers, feature/diagnostic code, and gate tables. All numeric comparisons are **round-before-compare** at the declared precision in **III.json**.
...
$$
\begin{aligned}
\mathrm{Canon}_{\mathcal G}&:\text{bytes}\to\text{canonical tables},\\
...
2. (**Gauge invariance**) For any admissible re-description $g\in\mathcal G$, $\delta(E)=\delta(E^g)$ where $E^g$ applies $g$ to inputs; likewise $y$ is invariant up to rounding.
...
(2) $\mathrm{Canon}_{\mathcal G}$ quotients inputs by $\mathcal G$; diagnostics operate on equivalence classes, hence invariance.
...
**Lemma C (gauge invariance under $\mathcal G$).**
For admissible $g\in\mathcal G$, the tuple of rounded diagnostics is unchanged.
...
*Sketch.* “All” is implemented as open-join reproducibility under $\mathcal G$, not as sample coverage; floors + TTDA + RFD define universality operationally.
...
If two labs publish editions $E_1,E_2$ with identical **III.json**, and their Receipt bundles differ only by raw bytes and benign $\mathcal G$-re-descriptions, then any admissible observer reproduces $\delta(E_1)=\delta(E_2)$ when $\Phi_{\mathrm{eff}}$ and floors/TTDA agree after rounding.
*Sketch.* Lemmas A–D.
...
**Purpose.**
The G-floor enforces **gauge invariance under admissible re-descriptions**: if inputs are equivalently expressed (units, encodings, orderings, tokenization, locale, benign paraphrase of captions), then all **rounded diagnostics that feed $\Phi$** must be unchanged. This appendix specifies (i) the *classes* of admissible re-description in $\mathcal G$, (ii) the **canonicalization** map used by loaders, and (iii) the **test suite** that certifies invariance. Failures are floor violations (→ $\delta=-1$).

---

### B.1 Admissible re-description classes $\mathcal G$

We model $\mathcal G$ as a finite product of generators; an admissible re-description is any finite composition of the following transformations, each **semantics-preserving** at declared precision:
...
$\mathcal G = U \times N \times L \times E \times S \times C \times T \times K \times P$. Admissibility is *relative to* the pinned canonicalizer: if the transform is not recognized/pinned, it is **not** in $\mathcal G$.
...
$$
\mathrm{Canon}_{\mathcal G}:\ \text{bytes}\ \to\ \text{canonical table(s)}
$$
...
**Goal.** Prove that for any $g\in\mathcal G$, the rounded diagnostic vector $\mathrm{ROUND}(\text{diag},\rho)$ is unchanged.
...
* **Transform bank.** Implement each generator in $\mathcal G$: U: °C↔K, psi↔Pa, eV↔J; N: comma/dot, per-mille, exponent spellings; L: month/day in 6 languages; E: NFC/NFKC, BOM; S: row/col permutation; C: container flips; T: timezone/dst/epoch shims; K: tokenization variants; P: caption paraphrases with bound-table retained.
...
Transformations **not** in $\mathcal G$ must flip the invariance test, yielding explicit **floor violations**:
...
* **$\mathcal G$-fuzzer seeds.** Reproducible seeds and transform sequences for CI.
...
* **Pinned $\mathcal G$ spec.** The exact transform set and canonicalizer version are hashed and referenced in **III.json**.
* **Backwards compatibility.** Expanding $\mathcal G$ or canonicalizers is a **MAJOR** change; prior editions remain valid but must declare the version.
...
* [ ] Confirm $\mathcal G$ test bank availability; plan invariance sweep post-run.
...
* [ ] Execute invariance suite on admissible $\mathcal G$ transforms; expect equality of rounded diagnostics.
...
* **F-A G-floor FAIL** → Check unit map, locale parsing, key order; re-canonicalize; if transform not in $\mathcal G$, remove or extend $\mathcal G$ via MAJOR edition.
...
**Purpose.**
REPLAY-RFD (*Replay-to-First-Divergence*) is Gravastar’s byte-true audit primitive. It lets any admissible observer reproduce an edition’s pipeline inside the pinned container and report the **earliest** place where bytes (or rounded tables) cease to match the receipt. This appendix gives a procedural guide, comparison rules, divergence taxonomy, minimal counterexample packs, and worked exemplars. REPLAY-RFD is an audit/repro procedure—**not** the Cook–Mertz *Tree Evaluation* complexity problem.
...
* **D-GFL (gauge invariance).** A re-description in $\mathcal G$ (units, column order, tokenization, locale) yields a different rounded diagnostic ⇒ G-floor FAIL.
...
* *Implication.* Method is not invariant under admissible $\mathcal G$ transform; δ must be −1 until canonicalizer is fixed.
...
* **Harden canonicalizer** for all declared $\mathcal G$ transforms.
...
**Admissible observer.** Any observer who (i) conforms to the declared observer-equivalence group $\mathcal{G}$ and (ii) follows the public method; must reproduce rounded results within budgets. See §2.2.
...
**$\mathcal{G}$ (observer-equivalence group).** Declared set of admissible re-descriptions (paraphrase, ordering, units/encodings, tokenization, locale, etc.). Invariance under $\mathcal{G}$ is enforced by G-floor. See §2.2 and §5.1.

**G-floor (gauge invariance).** Floor ensuring invariance of rounded outcomes under all declared $\mathcal{G}$ transforms. See §5.1 and App. B.
...
**REPLAY-RFD.** Replay-to-First-Divergence audit primitive: walk the RFD graph inside the pinned container and stop at the earliest byte/rounded mismatch. Not the Cook–Mertz Tree Evaluation problem. See §7.3 and App. I.
...
**Universally admissible (“acceptable to all”).** Operational: any admissible observer (under $\mathcal{G}$) who follows the method reproduces the result within budgets; any compliant counterexample defeats it. See Abstract and §1.2.
